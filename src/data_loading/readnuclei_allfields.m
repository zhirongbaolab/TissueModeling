...
% creating time vector from file name. fileparts returns path name, file name and extension for the specified file
[pathstr, name] = fileparts(filename);
if (length(name) < 3)
    error('file name too short, could not extract the time point in which the file was taken')
end

% extract the time from the file name e.g. 001 from t001-nuclei
t = str2double(name(2:4));

% WHAT IS GOING ON HERE
T = t*ones(length(C{1}),1);

% getting direct successors - FIGURE THESE OUT
temp = C{4};
...
% the second field indicates where the cell at a given line is valid
valid_id =  find( C{2} == 1);
